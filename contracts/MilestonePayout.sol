...
abstract contract MilestonePayout is AgreementRegistry {
    ...
    event PaymentReleased(uint256 indexed agreementId, uint256 indexed index, address indexed carrier, uint256 amount);
    ...
    function setupMilestones(
        ...
        if (descriptions.length != a.milestoneCount || amounts.length != a.milestoneCount) {
     ...
     */
    function submitMilestone(uint256 agreementId, uint256 index)
        external
        whenNotPaused
        agreementExists(agreementId)
    {
        ...
        if (index >= agreementMilestones[agreementId].length) revert InvalidMilestoneIndex();
     ...
     */
    function verifyAndReleaseMilestone(uint256 agreementId, uint256 index)
        external
        whenNotPaused
        agreementExists(agreementId)
    {
